...
public class LoginUserServiceImpl implements LoginUserService {
    ...
    public boolean register(JSONObject object) {
        ...
        SmsValidateResult smsValidateResult = VerificationCodeService.validSmsValidateResult.get(msgId);
        if (smsValidateResult == null || !Objects.equals(object.getString("telephone"), smsValidateResult.getMobile().toString())){
            throw new BaseException(ErrorCodeEnum.PHONE_IS_DIFFERENT);
        }else {
            VerificationCodeService.validSmsValidateResult.remove(msgId);
        }
...
//        user.setUsername("0");
        ...
        user.setNickname(StringUtils.isBlank(object.getString("nickname")) ? object.getString("telephone") : object.getString("nickname"));
        user.setPassword(PasswordProvider.encrypt(Base64Util.decoder(object.getString("password"))));
    ...
    public boolean saveTenantCompany(JSONObject object, User user) {
        ...
        tenant.setTenantManager(user.getNickname());
