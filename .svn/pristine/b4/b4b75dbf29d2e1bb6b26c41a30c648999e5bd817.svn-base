...
import com.chejet.cloud.vo.CompanyManagerVO;
...
public class CompanyManagerController extends BaseController {
    ...
    @RequestMapping("remove/{companyId}/{appId}")
    public ApiResp remove(@PathVariable Long companyId, @PathVariable Long appId) throws Exception {
        boolean temp = companyService.deleteCompanyManager(companyId, appId, currentUser());
        if (temp == true) {
    ...
    public ApiResp list(@PathVariable Long companyId) throws Exception {
        List<CompanyManagerVO> voPage = companyService.listCompanyManager(companyId, currentUser());
        if (voPage != null) {
    ...
    public ApiResp edit(@PathVariable Long managerId, @PathVariable(required = false) Long appId, @RequestBody String payload) throws Exception {
